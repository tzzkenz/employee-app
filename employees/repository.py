...
async def create_address(
    employee: Employee, address: Address, db: AsyncSession
) -> Address:
...
async def get_address(address_id: int, db: AsyncSession) -> Address | None:
    statement = select(Address).where(
        Address.deleted_at.is_(None), Address.id == address_id
    )
    address = await db.scalars(statement)
    return address.one_or_none()


async def get_address_with_employee(
    employee_id: int, address_id: int, db: AsyncSession
) -> Address | None:
    statement = select(Address).where(
...
async def get_all_addresses(employee_id: int, db: AsyncSession) -> list[Address]:
    statement = select(Address).where(
        Address.deleted_at.is_(None), Address.employee_id == employee_id
    )
    result = await db.scalars(statement)
    return result.all()


async def get_employee_with_department(
...
async def delete_department_from_employee(
    employee: Employee, db: AsyncSession
) -> Employee:
    return await save(employee, db)
