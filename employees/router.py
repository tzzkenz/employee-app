...
async def patch_employee(employee_id: int, body: EmployeePatch, db: AsyncSession = Depends(get_db)):
...
async def get_all_addresses(employee_id: int,  db: AsyncSession = Depends(get_db)):
...
async def patch_address(address_id: int, body: AddressPatch, db: AsyncSession = Depends(get_db)):
