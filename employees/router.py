...
async def create_employee(body: EmployeeCreate, db: AsyncSession = Depends(get_db), current_user: TokenPayload = Depends(require_role(EmployeeRole.HR))):
...
async def get_employee(employee_id: int, db: AsyncSession = Depends(get_db), current_user: TokenPayload = Depends(require_role(EmployeeRole.HR) )):
  return await service.get_employee(employee_id, db)

@router.delete("/{employee_id}", response_model=EmployeeResponse)
async def delete_employee(employee_id: int, db: AsyncSession = Depends(get_db), current_user: TokenPayload = Depends(require_role(EmployeeRole.HR))):
  return await service.delete_employee(employee_id, db)

@router.patch("/{employee_id}", response_model=EmployeeResponse)
async def patch_employee(employee_id: int, body: EmployeePatch, db: AsyncSession = Depends(get_db), current_user: TokenPayload = Depends(require_role(EmployeeRole.HR))):
  return await service.patch_employee(employee_id, body, db)

@router.post("/{employee_id}/address", response_model=AddressResponse)
async def create_address(employee_id: int, body: AddressCreate, db: AsyncSession = Depends(get_db), current_user: TokenPayload = Depends(require_role(EmployeeRole.HR))):
...
async def delete_address(employee_id: int, address_id: int, db: AsyncSession = Depends(get_db), current_user: TokenPayload = Depends(get_current_user), TokenPayload = Depends(require_role(EmployeeRole.HR))):
...
async def patch_address(address_id: int, body: AddressPatch, db: AsyncSession = Depends(get_db), current_user: TokenPayload = Depends(require_role(EmployeeRole.HR))):
...
async def add_department_to_employee(employee_id: int, department_id: int, db: AsyncSession = Depends(get_db), current_user: TokenPayload = Depends(require_role(EmployeeRole.HR))):
...
async def delete_department_from_employee(employee_id: int, department_id: int, db: AsyncSession = Depends(get_db), current_user: TokenPayload = Depends(require_role(EmployeeRole.HR))):
