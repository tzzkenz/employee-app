...
async def get_department(department_id: int, db: AsyncSession) -> Department | None:
...
async def get_department_with_employees(department_id: int, db: AsyncSession) -> Department | None:
...
async def get_all_employees_in_department(department_id: int, db: AsyncSession) -> list[Employee]:
  statement = select(Employee).join(Employee.departments).where(Department.id == department_id, Department.deleted_at.is_(None), Employee.deleted_at.is_(None))
  result = await db.scalars(statement)
  return result.all()
